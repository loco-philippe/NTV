...
class TestNtvFast(unittest.TestCase):
    ...
    def test_from_obj_repr(self):
        ...
        for test in list_repr:
            # print(test)
            self.assertEqual(
                Ntv.fast(test[0]).to_repr(False, False, False, 10), test[1]
            )
    ...
    def test_equivalence(self):
        str2 = [
            (
                {"Ntv1": datetime.date(2021, 2, 1)},
                {"Ntv1:date": datetime.date(2021, 2, 1)},
            ),
            (
                {
                    "Ntv2": [
    ...
    def test_from_obj_fast(self):
        self.assertNotEqual(
            Ntv.fast({":": NtvSingle(1, "test")}), Ntv.fast(NtvSingle(1, "test"))
        )
        dictstr2 = [
            ...
            [
                "NtvSingle",
                {"set:": NtvList([{"l1": 21}, {"l2": datetime.date(2021, 2, 1)}])},
                {"set:ntv": {"l1": 21, "l2:date": "2021-02-01"}},
            ],
            [
                "NtvList",
                [[4, [5, 6]], {"heure": [datetime.time(10, 25, 10), 22]}],
            ...
            [
                ...
                [
                    [4, [5, 6]],
                    {"heure": [{":time": "10:25:10"}, {":point": [3.0, 4.0]}]},
                ],
            ],
            ["NtvList", [], {}],
            [
            ...
            [
                "NtvList",
                {"Ntv3::fr.reg": {"Ntv1": 1, "Ntv2:fr.reg": "2"}},
                {"Ntv3::fr.reg": {"Ntv1": 1, "Ntv2": "2"}},
            ],
            [
                "NtvList",
                {"Ntv3::fr.reg": {"Ntv1": [1, 2], "Ntv2:fr.reg": "2"}},
    ...
    def test_to_json_ntv_to_obj_ntv(self):
        ...
        for js in js_obj:
            ...
            self.assertEqual(
                Ntv.fast(js).to_json_ntv().to_obj_ntv(), Ntv.fast(js).to_obj_ntv()
            )


class TestNtvCreation(unittest.TestCase):
    """test NTV creation"""
    ...
    def test_from_obj_repr(self):
        ...
        for test in list_repr:
            # print(test)
            self.assertEqual(
                Ntv.from_obj(test[0]).to_repr(False, False, False, 10), test[1]
            )
            self.assertEqual(json.loads(repr(Ntv.fast(test[0]))), test[0])
        list_repr = [
            [{"truc:": 1}, "sN"],
    ...
    def test_from_att(self):
        self.assertEqual(
            repr(Ntv.obj(([[1, 2], [3, 4]], None, "point", "single"))),
            '{":point": [[1, 2], [3, 4]]}',
        )
        self.assertEqual(
            repr(Ntv.obj(([[1, 2], [3, 4]], None, "point", "list"))),
            '{"::point": [[1, 2], [3, 4]]}',
        )
    ...
    def test_from_obj_obj(self):
        self.assertEqual(
            NtvSingle(datetime.date(2021, 10, 1), None, Datatype("datetime")).type_str,
            "date",
        )
        self.assertNotEqual(
            Ntv.obj({":": NtvSingle(1, "test")}), Ntv.obj(NtvSingle(1, "test"))
        )
    ...
    def test_to_obj(self):
        ...
        self.assertTrue(
            Ntv.from_obj(nstr).to_obj(simpleval=True)
        ...
        self.assertEqual(
            Ntv.obj({"paris:point": "null"}).to_obj(format="obj"), {"paris:point": None}
        )


class TestNtvValidator(unittest.TestCase):
    """test NTV validate"""
...
class TestNtvCompare(unittest.TestCase):
    """test NTV compare"""
...
class TestNtvPointer(unittest.TestCase):
    """test NTV pointer"""
    ...
    def test_pointer_RFC(self):
        ...
        self.assertTrue(
            ntv["#/a/1/1"] == ntv["#/0/1/1"] == ntv[0][1][1] == ntv["a"][1][1]
        )
        self.assertTrue(
            ntv["#/dic/v1"] == ntv["#/2/0"] == ntv[2][0] == ntv["dic"]["v1"]
        )
        js = {
            "foo": ["bar", "baz"],
            " ": 0,
    ...
    def test_json_ntv_pointer(self):
        examples = [
            {"data": {"a": 1, "test": "ok"}, "pointer": "/test", "canonical": "/1"},
    ...
    def test_conversion_json_ntv_pointer(self):
        examples = [
            {"data": {"test": "ok"}, "pointer": "test", "pointer json": "/test"},
            {
                "data": [1, {"test": "ok"}],
                "pointer": "/test",
                "pointer json": "/1/test",
            },
            {
                "data": [1, [2, {"test": "ok"}]],
                "pointer": "/1/test",
                "pointer json": "/1/1/test",
            },
            {
                "data": [1, {"a": {"test": "ok"}}],
                "pointer": "/a/test",
                "pointer json": "/1/a/test",
            },
            {
                "data": {"b": [1, {"a": {"test": "ok"}}]},
                "pointer": "b/a/test",
                "pointer json": "/b/1/a/test",
            },
            {
                "data": {"b": [1, {"a": {"test": "ok"}}]},
                "pointer": "0/a/test",
        ...
        for example in examples:
            ...
            ntv = Ntv.obj(data)
            self.assertEqual(
                ntv["#" + pointer].val, resolve_pointer(data, pointer_json)
            )
            self.assertEqual(
                ntv["#" + pointer].pointer(),
            ...
            if pointer[0] != "0":
                self.assertEqual(
                    NtvUtil.to_ntvpointer(pointer_json, unique_root), pointer
                )
...
class TestNtvTabular(unittest.TestCase):
    ...
    def test_tab(self):
        ...
        self.assertEqual(
            tab[1][2], tab["dates::datetime"][2], Ntv.obj({":datetime": "2022-01-21"})
        )
...
class TestNtvFunction(unittest.TestCase):
    ...
    def test_obj_ntv(self):
        ...
        for tst in test_s:
            self.assertEqual(
                Ntv.obj_ntv(*tst, True),
                NtvSingle(*tst).to_obj(),
    ...
    def test_cast(self):
        ...
        for obj in list_obj:
            self.assertEqual(Ntv.from_obj(NtvSingle(obj).to_obj()), NtvSingle(obj))
    ...
    def test_agreg_type(self):
        ...
        for typ in list_type:
            ...
            if typ[0] == [None, None, False]:
                self.assertEqual(agreg_type(typ[0][0], typ[0][1], typ[0][2]), typ[1])
            else:
                self.assertEqual(
                    agreg_type(typ[0][0], typ[0][1], typ[0][2]).long_name, typ[1]
                )

    def test_default_type(self):
        list_test = [
            [("", ":", "fr.BAN.lon"), {"ntv1::fr.BAN.": [{":BAN.lon": 4}, 5, 6]}],
        ...
        for test in list_test:
            # print(test[1])
            self.assertEqual(
                Ntv.from_obj(test[1]).ntv_value[0].json_name(), list(test[0])
            )
            self.assertEqual(Ntv.obj(test[1]).ntv_value[0].json_name(), list(test[0]))
...
class TestNtvTree(unittest.TestCase):
    """test NTV tree"""
    ...
    def test_NtvTree(self):
        ...
        self.assertEqual(
            ...
            [str(node.pointer()) for node in tree.leaf_nodes][6],
            "/b",
        )
...
class Test_NtvConnector(unittest.TestCase):
    def test_is_json(self):
        ...
        self.assertTrue(
            is_json({"tst": [1, 2, "test", None, True, {"test": 1}, [1, "tst"]]})
        )
        self.assertFalse(
            is_json(
                {
        ...
        with self.assertRaises(NtvError):
            is_json(
                {"tst": [1, 2, "test", None, True, {"test": NtvTree(None)}, [1, "tst"]]}
            )

    def test_cast_uncast(self):
        self.assertEqual(
            NtvConnector._typ_obj(
                [
                    21,
                    ["ser", 25],
                    [datetime.date(2020, 3, 4), datetime.date(2020, 4, 4)],
            ...
            NtvConnector._typ_obj(datetime.date(2020, 3, 4)),
            "date",
        )
        self.assertEqual(
            NtvConnector._typ_obj(
...
class TestNtvPatch(unittest.TestCase):
    """test NTV patch"""
    ...
    def test_op(self):
        a = Ntv.obj(
            {
                "test": [[1, 2, 3], {"liste": [0, 1, 2, 0, 1, {"val": [1, 2]}]}],
                "truc": 1,
            }
        )
        entity = {"new": "entity"}
    ...
    def test_init_patch(self):
        ...
        pat2 = NtvPatch(
            {"list-op": [NtvOp(js_cop), NtvOp(js_tst), js_del], "comment": "test"}
        )
        self.assertTrue(pat1 == pat2)
...
class TestNtvComment(unittest.TestCase):
    """test NTV comment"""
    ...
    def test_comment(self):
        ...
        com.add(
            {
                "comment": "a corriger suite",
                "list-op": [
                    {
                        "op": "replace",
                        "path": "/dates/1",
                        "entity": {":date": "1998-02-05"},
                    }
                ],
            }
        )
