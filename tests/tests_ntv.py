...
class TestNtvFast(unittest.TestCase):
    ...
    def test_from_obj_repr(self):
        ...
        for test in list_repr:
            # print(test)
            self.assertEqual(Ntv.fast(test[0]).to_repr(
                False, False, False, 10), test[1])
    ...
    def test_equivalence(self):
        str2 = [({'Ntv1': datetime.date(2021, 2, 1)}, {'Ntv1:date': datetime.date(2021, 2, 1)}),
                ({'Ntv2': [datetime.date(2020, 2, 4), [datetime.date(2020, 3, 4),
                                                       datetime.date(2020, 4, 4)]]},
                 {'Ntv2:date': [datetime.date(2020, 2, 4), [datetime.date(2020, 3, 4),
                                                            datetime.date(2020, 4, 4)]]}),
                ]
        for js1, js2 in str2:
            self.assertEqual(Ntv.fast(js1).to_json_ntv(),
                             Ntv.fast(js1).to_json_ntv())
    ...
    def test_from_obj_fast(self):
        ...
        for nstr, typ, nres in zip(liststr, listtyp, listres):
            ...
            # print('ap', nstr, typ)
            self.assertTrue(ntv.__class__.__name__ == typ)
            self.assertEqual(ntv.to_obj_ntv(), Ntv.fast(
                ntv.to_fast()).to_obj_ntv())
    ...
    def test_to_json_ntv_to_obj_ntv(self):
        ...
        for js in js_obj:
            ...
            self.assertEqual(Ntv.fast(js).to_json_ntv(
            ).to_obj_ntv(), Ntv.fast(js).to_obj_ntv())
...
class TestNtvCreation(unittest.TestCase):
    ...
    def test_from_obj_repr(self):
        ...
        for test in list_repr:
            # print(test)
            self.assertEqual(Ntv.from_obj(test[0]).to_repr(
                False, False, False, 10), test[1])
            self.assertEqual(json.loads(repr(Ntv.fast(test[0]))), test[0])
        list_repr = [[{"truc:":  1}, "sN"],
                     [{":": 1}, "s"],
    ...
    def test_from_obj_obj(self):
        self.assertEqual(NtvSingle(datetime.date(2021, 10, 1), None,
                                   Datatype('datetime')).type_str, 'date')
        self.assertNotEqual(
...
class TestNtvCompare(unittest.TestCase):
    ...
    def test_lt(self):
        ...
        self.assertTrue(Ntv.obj({'test': 2, 'r': 4}) <
                        Ntv.obj({'test': 2, 'res': 5}))
        self.assertFalse(Ntv.obj({'test': 2, 'r': 4})
                         < Ntv.obj({'test': 2, 'res': 1}))
        self.assertFalse(Ntv.obj({'test': 2, 'r': 4}) < Ntv.obj({'test': 2}))
        self.assertTrue(Ntv.obj({'test': 2}) < Ntv.obj({'test': 2, 'res': 1}))
        self.assertTrue(Ntv.obj({'test': 2, 'tr': 4})
                        > Ntv.obj({'test': {'truc': 2}}))
...
class TestNtvPointer(unittest.TestCase):
    ...
    def test_pointer_RFC(self):
        ...
        self.assertTrue(ntv['#/a/1/1'] == ntv['#/0/1/1']
                        == ntv[0][1][1] == ntv['a'][1][1])
        self.assertTrue(ntv['#/dic/v1'] == ntv['#/2/0']
                        == ntv[2][0] == ntv['dic']['v1'])
    ...
    def test_json_ntv_pointer(self):
        ...
        for example in examples:
            ...
            self.assertEqual(ntv['#'+pointer].val,
                             resolve_pointer(data, pointer))

    def test_conversion_json_ntv_pointer(self):
        examples = [{'data': {'test': 'ok'},                    'pointer': 'test',
                     'pointer json': '/test'},
                    {'data': [1, {'test': 'ok'}],               'pointer': '/test',
                     'pointer json': '/1/test'},
                    {'data': [1, [2, {'test': 'ok'}]],          'pointer': '/1/test',
                     'pointer json': '/1/1/test'},
                    {'data': [1, {'a': {'test': 'ok'}}],        'pointer': '/a/test',
                     'pointer json': '/1/a/test'},
                    {'data': {'b': [1, {'a': {'test': 'ok'}}]}, 'pointer': 'b/a/test',
                     'pointer json': '/b/1/a/test'},
                    {'data': {'b': [1, {'a': {'test': 'ok'}}]}, 'pointer': '0/a/test',
                     'pointer json': '/b/1/a/test'},
                    {'data': {'b': {'a': 1, 'test': 'ok'}},     'pointer': 'b/test',
                     'pointer json': '/b/test'},
                    {'data': {'a': 1, 'test': 'ok'},            'pointer': '/test',
                     'pointer json': '/test'},
                    {'data': [1, {'a': {'test': [1, 'ok']}}],    'pointer': '/a/test/1',
        ...
        for example in examples:
            ...
            ntv = Ntv.obj(data)
            self.assertEqual(ntv['#'+pointer].val,
                             resolve_pointer(data, pointer_json))
            self.assertEqual(ntv['#'+pointer].pointer(),
                             ntv['#'+NtvUtil.to_ntvpointer(pointer_json, unique_root)].pointer())
            if pointer[0] != '0':
                self.assertEqual(NtvUtil.to_ntvpointer(
                    pointer_json, unique_root), pointer)
...
class TestNtvFunction(unittest.TestCase):
    ...
    def test_obj_ntv(self):
        l_val_s = [{'tst': [1, 2, 3]}, {'tst': 1},  {
            'tst': [1, 2, 3], 'tst2': 5}, [1, 2, 3], 5, 'test']
        l_name = ['tst', '']
        l_typ = ['int32', '']
...
class Test_NtvConnector(unittest.TestCase):
    ...
    def test_is_json(self):
        ...
        with self.assertRaises(NtvError):
            is_json({'tst': [1, 2, 'test', None, True, {
                    'test': NtvTree(None)}, [1, 'tst']]})
    ...
    def test_cast_uncast(self):
        ...
        for obj in list_obj:
            self.assertEqual(obj, NtvConnector.uncast(
                *NtvConnector.cast(obj))[0])
...
class TestNtvPatch(unittest.TestCase):
    ...
    def test_op(self):
        a = Ntv.obj(
            {'test': [[1, 2, 3], {'liste': [0, 1, 2, 0, 1, {'val': [1, 2]}]}], 'truc': 1})
        entity = {'new': 'entity'}
    ...
    def test_init_patch(self):
        ...
        js_cop_com = {'op': 'copy', 'from': '/0/1/1',
                      'path': '/0/1/3', 'comment': 'test'}
        ...
        pat3 = NtvPatch(
            {'list-op': [NtvOp(js_cop), NtvOp(js_tst), NtvOp(js_del)]})
