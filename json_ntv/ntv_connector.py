...
def to_csv(
    file_name, ntv, *args, restval="", extrasaction="raise", dialect="excel", **kwds
):
    ...
    *parameters*
    ...
    with open(file_name, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for i in range(len(list_ntv[0])):
            writer.writerow(
                {
                    name: field_ntv[i].to_obj(field_ntv.ntv_type, encoded=True)
...
class ShapelyConnec(NtvConnector):
    ...
    def to_obj_ntv(ntv_value, **kwargs):
        ...
        *Parameters*
        ...
        type_geo = (
            ShapelyConnec.type_geo(ntv_value)
            if "type_geo" not in kwargs or kwargs["type_geo"] == "geometry"
            else kwargs["type_geo"]
        )
    ...
    def to_geometry(value):
        ...
        return ShapelyConnec.to_obj_ntv(
            value, type_geo=NtvConnector.DIC_GEO[ShapelyConnec.type_geo(value)]
        )
...
class MermaidConnec(NtvConnector):
    ...
    def _mermaid_node(ntv, def_typ_str, num, dic_node, ind):
        ...
        if num:
            ...
            return [
                str(ntv.pointer(index=True, item_idx=ind)),
                ["rectangle", name[:-1]],
            ]
        if not name:
            name = "<b>::</b>\n"
        name = name.replace('"', "'")
        return [str(ntv.pointer(index=True, item_idx=ind)), ["roundedge", name[:-1]]]
    ...
    def _mermaid_link(ntv, def_typ_str, node_link, row, dic_node, indic):
        ...
        node_link["nodes"].append(
            MermaidConnec._mermaid_node(ntv, def_typ_str, num, dic_node, indic)
        )
        if isinstance(ntv, NtvList):
            for ind, ntv_val in enumerate(ntv):
                MermaidConnec._mermaid_link(
                    ntv_val, ntv.type_str, node_link, row, dic_node, ind
                )
                node_link["links"].append(
                    [
                        str(ntv.pointer(index=True)),
                        "normalarrow",
