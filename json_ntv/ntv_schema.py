...
def val_items(ntv_data, sch, mode) : 
    return validat(sh_expand(ntv_data), sh_items(sch), mode)
...
def val_simple(ntv_data, sch, mode) :
    ...
    valid = True
    valid &= validat(json_data['value'], sh_compact(sch)['value'], mode)
    valid &= validat(json_data['type'],  sh_compact(sch)['type'], mode)
    valid &= validat(json_data['name'],  sh_compact(sch)['name'], mode)
    return valid
...
def ntv_validate(data, sch, mode=0):
    ...
    for ntv in list(ntv_data.tree)[1:]:
        ...
        #print('entity : ', new_p_data,  ' parent : ', str(parent_ntv), mapping[str(parent_ntv)])
