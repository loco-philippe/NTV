...
URI = re.compile(r"^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\?([^#]*))?(#(.*))?")
UUID = re.compile(
    "[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}", flags=re.IGNORECASE
)
...
ADDRESS = re.compile(
    _MAILBOX + "|(.*:(" + _MAILBOX + "(," + _MAILBOX + r")*)?;\s*)"
)  # without CFWS
...
class Validator:
    ...
    def dat_valid(val):
        """validate method"""
        return (
            Validator.date_valid(val)
            or Validator.time_valid(val)
            or Validator.datetime_valid(val)
            or Validator.timetz_valid(val)
            or Validator.datetimetz_valid(val)
        )
    ...
    def period_valid(val):
        ...
        for per in period:
            if not (
                Validator.datetime_valid(per)
                or Validator.datetimetz_valid(per)
                or Validator.duration_valid(per)
            ):
                return False
    ...
    def timearray_valid(val):
        ...
        return (
            isinstance(val, list)
            and len(val) == 2
            and Validator.dat_valid(val[0])
            and Validator.dat_valid(val[1])
        )
    ...
    def geometry_valid(val):
        """validate method"""
        return (
            Validator.point_valid(val)
            or Validator.line_valid(val)
            or Validator.polygon_valid(val)
        )
    ...
    def loc_valid(val):
        ...
        return (
            Validator.point_valid(val)
            or Validator.pointstr_valid(val)
            or Validator.pointobj_valid(val)
            or Validator.line_valid(val)
            or Validator.polygon_valid(val)
            or Validator.multipolygon_valid(val)
            or Validator.box_valid(val)
            or Validator.geojson_valid(val)
            or Validator.codeolc_valid(val)
        )
