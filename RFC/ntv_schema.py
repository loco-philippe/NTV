...
def _val_prop(ntv_data, sch, mode):
    ...
    for idx, ntv in enumerate(ntv_data):
        p_name = (
            ntv.ntv_name
            if ntv.ntv_name in sch
            else (ntv.json_name_str if ntv.json_name_str in sch else None)
        )
...
def _val_required(ntv_data, sch, mode):
    ...
    return validat(
        [ptr[1:] for ptr in sch], valid, "required", ntv_data.pointer(), mode
    )
...
def _val_simple(ntv_data, sch, mode):
    ...
    for part in ["valueNtv", "typeNtv", "nameNtv"]:
        valid &= validat(
            json_data[part], json_sch[part], part, ntv_data.pointer(), mode
        )
    return valid
...
def _simple_to_jsch(ntvsch):
    ...
    jsonsch["valueNtv"] |= {
        key: val
        for key, val in ntvsch.items()
        if key not in ["typeNtv", "nameNtv", "valueNtv", "propertyNames"]
    }
...
def _json(dic):
    ...
    return (
        {key[1:] if key[0] == ":" else key: _json(value) for key, value in dic.items()}
...
def _simp(dic, keywords=None):
    ...
    return (
        {key: val for key, val in dic.items() if key not in keyw}
        if isinstance(dic, dict)
        else dic
    )
...
def _json2(dic):
    """in a schema leaf, transform NTVkeywords into JSONkeywords"""
    return (
        {_replace(key): _json2(value) for key, value in dic.items()}
        if isinstance(dic, dict)
        else dic
    )
